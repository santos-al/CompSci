...
When you double-click on a file to open it, your computer uses its file extension to 
...
that server sends an HTTP header, along with the file itself, indicating the file’s media type. For instance, 
the media type for a GIF is image/gif, and the media type for a JPEG is image/jpeg. To determine the media type for a file,
...
file = str(input("Please input your file name. ")).lower().strip()
